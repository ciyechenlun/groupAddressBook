CreatePublicRoad=function(){
	return{
		...
		toStepTwo:function(){
			...
			if(toRange=='1'){
				CreatePublicRoad.departmentTree(companyId);
				$(".othChoice").hide();
				$(".comChoice").show();
			}else if(toRange=='2'){
				CreatePublicRoad.userTree(companyId);
				$(".othChoice").hide();
				$(".comChoice").show();
			}else{
				$(".comChoice").hide();
...
//	                        node.target.nextSibling.style.display=none;
				...
				onCheck:function(node,checked){
					if(checked){
                        ...
                        CreatePublicRoad.addMember(node.id, node.text);
					}else{
						if(!$('#targetTree').tree('isLeaf',node.target)){
							var childrenNode = $("#targetTree").tree('getChildren', node.target);  
						...
						CreatePublicRoad.delMember(node.id);
					}			
				}
			});
...
//	                        node.target.nextSibling.style.display=none;
		...
		addMember:function(id,name){
			...
			var txt = '<li id='+id+'><a href="#" onclick="CreatePublicRoad.delMemberImg(\''+id+'\');"> <img src="/resources/images/icon_cancel.png"/></a>'+name+'</li>';
		...
		toStepThree:function(){
			...
			$("#step3").show();
			var companyId =  window.parent.document.getElementById("companyId").value;
			CreatePublicRoad.managerTree(companyId);
		},
...
//	                        node.target.nextSibling.style.display=none;
				...
				onCheck:function(node,checked){
					...
					if($('#managerTree').tree('isLeaf',node.target)){
						CreatePublicRoad.addManagerMember(node.id, node.text);
					} 
						...
						CreatePublicRoad.delManagerMember(node.id);
		...
		addManagerMember:function(id,name){
			...
			var txt = '<li id='+id+'><a href="#" onclick="CreatePublicRoad.delManagerMemberImg(\''+id+'\');"> <img src="/resources/images/icon_cancel.png"/></a>'+name+'</li>';
