package com.cmcc.zysoft.sellmanager.model;
// Generated 2013-5-22 21:25:25 by Hibernate Tools 3.2.2.GA


import javax.persistence.Column;
import javax.persistence.Entity;
import javax.persistence.GeneratedValue;
import javax.persistence.Id;
import javax.persistence.Table;

import org.hibernate.annotations.GenericGenerator;

/**
 * Upleader generated by hbm2java
 */
@Entity
@Table(name="tb_c_upleader")
public class Upleader  implements java.io.Serializable {


     /**
	 * 属性名称：serialVersionUID <br/>
	 * 类型：long
	 */
	 private static final long serialVersionUID = -4881608112900021765L;
	 private String upleaderId;
     private String departmentId;
     private String employeeId;
     private String mark1;
     private String mark2;

    public Upleader() {
    }

	
    public Upleader(String upleaderId) {
        this.upleaderId = upleaderId;
    }
    public Upleader(String upleaderId, String departmentId, String employeeId, String mark1, String mark2) {
       this.upleaderId = upleaderId;
       this.departmentId = departmentId;
       this.employeeId = employeeId;
       this.mark1 = mark1;
       this.mark2 = mark2;
    }
   
    @GenericGenerator(name = "generator", strategy = "uuid.hex")
	@Id
	@GeneratedValue(generator = "generator")
    @Column(name="upleader_id", unique=true, nullable=false, length=32)
    public String getUpleaderId() {
        return this.upleaderId;
    }
    
    public void setUpleaderId(String upleaderId) {
        this.upleaderId = upleaderId;
    }
    
    @Column(name="department_id", length=32)
    public String getDepartmentId() {
        return this.departmentId;
    }
    
    public void setDepartmentId(String departmentId) {
        this.departmentId = departmentId;
    }
    
    @Column(name="employee_id", length=32)
    public String getEmployeeId() {
        return this.employeeId;
    }
    
    public void setEmployeeId(String employeeId) {
        this.employeeId = employeeId;
    }
    
    @Column(name="mark1", length=200)
    public String getMark1() {
        return this.mark1;
    }
    
    public void setMark1(String mark1) {
        this.mark1 = mark1;
    }
    
    @Column(name="mark2", length=200)
    public String getMark2() {
        return this.mark2;
    }
    
    public void setMark2(String mark2) {
        this.mark2 = mark2;
    }




}


